...
(* This version of quotient has a [] in the definition,
 and uses (Classical Prop) for the set so that equality works better *)
...
Module Quotient (EqRel : EqRel).
  ...
  Definition t : Type :=
    {S : A -> Classical Prop |
      (forall x y, toProp (S x) -> ([R x y] <-> toProp (S y))) /\ [exists a, toProp (S a)]}.
  ...
  Definition mk (a : A) : t.
    refine (exist _ (fun a' => Creturn (R a' a)) _).
    subst.
    split.
    - intros.
      split.
  ...
  Proof.
    intros.
    destruct x as [S [resp ex]].
    assert (ex2 := ex).
    classical_auto.
    specialize ex2 as [a Sa].
    apply Preturn.
    exists a.
    apply sigEq2.
    simpl.
  ...
  (* this should work in this version *)
  Theorem unwrap_eq : forall (x y : t), [x = y] -> x = y.
  Proof.
    intros.
    apply sigEq2.
    destruct x, y.
